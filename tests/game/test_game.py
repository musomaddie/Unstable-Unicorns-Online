...
class TestTakeTurn:
    ...
    def test_turn_draws_card(self, game: Game):
        ...
        starting_hand_size = len(game.players.players[0].hand)
        ...
        assert len(game.players.players[0].hand) == starting_hand_size + 2
        assert len(game.deck) == starting_deck_size - 2
        assert game.players.players[0].hand[0] == top_deck_card
    ...
    def test_action_phase_draw(self, game: Game):
        ...
        assert len(game.players.players[0].hand) == starting_hand_size + 2
        assert len(game.deck) == starting_deck_size - 2
        assert game.players.players[0].hand[1] == second_deck_card
    ...
    def test_discardPhase_onlyCurrentPlayer(self, game: Game, fake_card: Card):
        ...
        assert len(game.players.players[0].hand) == 7
        assert len(game.players.players[1].hand) == 8
        assert len(game.players.players[2].hand) == 8
